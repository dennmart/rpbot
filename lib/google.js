  ...
  https.get(analyticsOptions, function(res) {
    ...
    res.on('end', function() {
      ...
      parseString(data, function (err, result) {
        ...
        var allTotal = 0;
        var percentage = 0.0;
        var response = result.feed.entry;

        response.each(function(entry) {
          if (entry.title.first()['_'] == "ga:browser=Internet Explorer | ga:browserVersion=7.0") {
            ie7Total = parseInt(entry['dxp:metric'].first()['$'].value);
          }
          allTotal += parseInt(entry['dxp:metric'].first()['$'].value);
        });
...
exports.googleImage = function(query, room) {
  ...
  var getRequest = https.get(googleImagesOptions, function(res) {
    ...
    res.on('end', function() {
      ...
      if (!Object.isEmpty(json.responseData.results)) {
        var imageInfo = json.responseData.results.randomize()[0];
        room.speak(imageInfo.unescapedUrl);
      } else {
        room.speak("Hmm... Seems like I couldn't find an image for that.");
      }
    });
